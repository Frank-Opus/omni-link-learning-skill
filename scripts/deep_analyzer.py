...
def gen_deep_analysis(text: str) -> str:
    """增强版深度分析 - 从原文智能提取（大量输出）"""
    s = "## 💡 深度分析与洞察\n\n### 底层逻辑分析\n\n**为什么有效？**\n\n"
    
    sentences = split_sentences(text)
    
    # Human nature insights - 增强版
    human_kws = ["人", "人性", "心理", "需要", "想要", "害怕", "觉得", "感觉", "希望", "渴望", "喜欢", "讨厌", "期待", "用户", "消费者"]
    human = [x for x in sentences if any(k in x for k in human_kws) and 40 < len(x) < 300]
    s += "1. **人性层面** (从原文提取 {0} 条洞察):\n\n".format(len(human))
    if human:
        for i, h in enumerate(human[:12], 1):  # 增加到 12 条
            s += f"   **{i}**. {h}\n\n"
    else:
        s += "   - 满足基本需求：被看见、被认可\n"
    s += "\n"
    ...
    # Business logic - 增强版
    biz_kws = ["价值", "利益", "成本", "收益", "效率", "利润", "赚钱", "生意", "资源", "交换", "买卖", "商业", "市场", "竞争", "壁垒", "护城河"]
    biz = [x for x in sentences if any(k in x for k in biz_kws) and 40 < len(x) < 300]
    s += "2. **商业/价值层面** (从原文提取 {0} 条洞察):\n\n".format(len(biz))
    if biz:
        for i, b in enumerate(biz[:12], 1):  # 增加到 12 条
            s += f"   **{i}**. {b}\n\n"
    else:
        s += "   - 创造价值，解决痛点\n"
    s += "\n"
    ...
    # System thinking - 增强版
    sys_kws = ["系统", "循环", "杠杆", "规模", "模式", "生态", "平台", "网络", "复制", "放大", "复利", "组织", "团队", "公司", "产品"]
    sys = [x for x in sentences if any(k in x for k in sys_kws) and 40 < len(x) < 300]
    s += "3. **系统/模式层面** (从原文提取 {0} 条洞察):\n\n".format(len(sys))
    if sys:
        for i, x in enumerate(sys[:12], 1):  # 增加到 12 条
            s += f"   **{i}**. {x}\n\n"
    else:
        s += "   - 系统性杠杆或可复制模式\n"
    s += "\n"
    
    # Pattern recognition - 增强版
    s += "### 模式识别\n\n**反映的更大模式？**\n\n"
    formulas = []
    for p in [r'叫做 (?:一个)?([ 的 A-Za-z0-9+\-]+(?:公式 | 法则 | 方法 | 策略 | 模式 | 理论 | 模型))',
              r'(?:第一 | 第二 | 第三 | 第四 | 第五)[、.]\s*(.+?)[.!?。！？]',
              r'(?:方法是 | 方式是 | 做法是 | 逻辑是 | 本质是)[:：\s]*(.+?)[.!?。！？]']:
        for m in re.findall(p, text):
    ...
    if formulas:
        s += "**提取的公式/模式/方法论** ({0} 个):\n\n".format(len(formulas))
        for i, f in enumerate(formulas[:15], 1):  # 增加到 15 个
            s += f"**{i}**. {f}\n\n"
        s += "\n"
    else:
        s += "**提取的公式/模式**: 从内容中识别重复出现的方法论和思维框架\n\n"
    ...
    # Cross-case comparison - 增强版
    s += "### 跨案例对比\n\n**与其他案例的异同？**\n\n"
    
    unique = [x for x in sentences if any(k in x for k in ["不是", "而不是", "不同于", "关键是", "核心", "相比", "对比", "差异", "区别"]) and 50 < len(x) < 300]
    
    s += "**对比维度分析**:\n\n"
    if unique:
        for i, u in enumerate(unique[:10], 1):  # 增加到 10 条对比
            s += f"**{i}**. {u}\n\n"
    else:
        s += "| 维度 | 本案例 | 典型做法 | 差异分析 |\n|------|--------|----------|----------|\n"
        s += "| 方法 | 从内容提取 | 常规做法 | 待对比 |\n"
    ...
    s += "\n**横向对比总结**:\n"
    s += "- **方法差异**: 本案例采用的独特方法与常规做法的对比\n"
    s += "- **效果差异**: 实际效果与预期效果的差距分析\n"
    s += "- **适用边界**: 方法适用的场景和限制条件\n\n"
    
    # 新增：思维框架提取
    s += "### 思维框架与心智模型\n\n"
    framework_kws = ["框架", "模型", "思维", "认知", "视角", "角度", "维度", "层面", "逻辑", "方法论"]
    frameworks = [x for x in sentences if any(k in x for k in framework_kws) and 50 < len(x) < 300]
    
    if frameworks:
        s += "**提取的思维框架** ({0} 个):\n\n".format(len(frameworks))
        for i, f in enumerate(frameworks[:10], 1):
            s += f"**{i}**. {f}\n\n"
    else:
        s += "**思维框架**: 从内容中识别分析问题的思维工具和框架\n\n"
    
    # 新增：决策依据提取
    s += "### 决策依据与判断标准\n\n"
    decision_kws = ["判断", "决策", "选择", "标准", "依据", "原则", "考虑", "因素", "评估", "权衡"]
    decisions = [x for x in sentences if any(k in x for k in decision_kws) and 50 < len(x) < 300]
