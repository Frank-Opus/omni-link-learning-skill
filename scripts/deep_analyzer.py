...
def gen_deep_analysis(text: str) -> str:
    ...
    s += "**核心方法论**: 不堆砌原文，只给你 speaker 真正想表达的洞察\n\n"
    s += "---\n\n"
    
    # ========== 核心洞察 Top 5 ==========
    s += "### 🎯 核心洞察 Top 5 (本集最重要的 5 件事)\n\n"
    
    # 提取最重要的洞察
    key_topics = []
    
    # 1. 巨头竞争观
    giant = [x for x in sentences if "巨头" in x and ("做" in x or "竞争" in x)]
    if giant:
        key_topics.append(("巨头进场怎么办？", giant[0]))
    
    # 2. 豆包预测
    doubao = [x for x in sentences if "豆包" in x and ("DAU" in x or "预测" in x or "27 年" in x)]
    if doubao:
        key_topics.append(("豆包预测", doubao[0]))
    
    # 3. 微信 AI
    wechat = [x for x in sentences if "微信" in x and "AI" in x]
    if wechat:
        key_topics.append(("微信 AI", wechat[0]))
    
    # 4. AI 硬件
    hardware = [x for x in sentences if ("硬件" in x or "豆包手机" in x) and ("期待" in x or "26 年" in x)]
    if hardware:
        key_topics.append(("AI 硬件机会", hardware[0]))
    
    # 5. 组织变革
    org = [x for x in sentences if ("组织" in x or "团队" in x) and ("AI" in x or "公司" in x)]
    if org:
        key_topics.append(("新公司形态", org[0]))
    
    for i, (topic, insight) in enumerate(key_topics[:5], 1):
        analysis = analyze_insight_deep(insight, text)
        s += f"**{i}. {topic}**\n\n"
        s += f"**一句话总结**: {analysis['一句话总结']}\n\n"
        s += f"**为什么重要**: {analysis['为什么重要']}\n\n"
        s += f"**具体怎么做**: {analysis['具体怎么做']}\n\n"
        s += "---\n\n"
    
    # ========== 2026 年具体机会 ==========
    s += "### 🚀 2026 年具体机会 (speaker 明确提到的)\n\n"
    
    opps = []
    
    # AI 硬件
    hw = [x for x in sentences if ("硬件" in x or "手机" in x) and ("26 年" in x or "期待" in x)]
    if hw:
        opps.append(("AI 硬件化", hw[0], "做一个'能帮我把事情搞定'的 AI 硬件，豆包手机思路正确"))
    
    # 多模态/可交互
    multi = [x for x in sentences if ("多模态" in x or "可交互" in x or "实时" in x) and ("26 年" in x or "突破" in x)]
    if multi:
        opps.append(("多模态可交互内容", multi[0], "短剧/直播/漫画的实时可交互产品"))
    
    # 微信 AI
    if wechat:
        opps.append(("微信 AI 功能", wechat[0], "关注微信 AI 的多模态功能，可能是虚拟人/数字人"))
    
    # ACGN 重做
    acgn = [x for x in sentences if "ACGN" in x or ("重做" in x and ("短剧" in x or "漫画" in x))]
    if acgn:
        opps.append(("ACGN 重做", acgn[0], "用 AI 重做短剧、直播、漫画"))
    
    # 录音+AI 分析
    record = [x for x in sentences if ("录音" in x or "记录" in x) and ("AI" in x or "分析" in x)]
    if record:
        opps.append(("AI 语音分析", record[0], "不是录音笔，是能理解内容的智能助手"))
    
    for i, (name, quote, action) in enumerate(opps[:5], 1):
        s += f"**{i}. {name}**\n\n"
        s += f"**speaker 原话**: {quote}\n\n"
        s += f"**机会在哪**: {action}\n\n"
        s += "---\n\n"
    
    # ========== 认知误区纠正 ==========
    s += "### ⚠️ 认知误区纠正 (speaker 指出的常见错误)\n\n"
    
    mistakes = []
    
    # 巨头恐惧症
    if giant:
        mistakes.append(("误区：巨头进场就完了", "正解：巨头愿意投入说明方向够大，跑得比内部团队快就能赢"))
    
    # 线性外推
    linear = [x for x in sentences if "线性外推" in x]
    if linear:
        mistakes.append(("误区：用线性思维判断 AI 发展", "正解：AI 是指数级增长，算力需求无穷 + 成本下降"))
    
    # 泡沫担忧
    bubble = [x for x in sentences if "泡沫" in x and ("没有意义" in x or "伪问题" in x)]
    if bubble:
        mistakes.append(("误区：纠结是不是泡沫", "正解：每个周期都有泡沫，关键是找到最终会赢的公司"))
    
    # Character.AI 高估
    if "Character" in text or "Cary.AI" in text:
        mistakes.append(("误区：Character.AI 能到数亿 DAU", "正解：它本质是 AI 不是角色本身，用户想要真角色"))
    
    # 用户创造力低估
    creativity = [x for x in sentences if ("创造力" in x or "低估" in x) and ("用户" in x or "普通" in x)]
    if creativity:
        mistakes.append(("误区：普通用户创造不出优质内容", "正解：抖音证明了产品结构开放后会涌现新范式"))
    
    for i, (mistake, correction) in enumerate(mistakes[:5], 1):
        s += f"**{i}. {mistake}**\n\n"
        s += f"**✅ {correction}**\n\n"
        s += "---\n\n"
    ...
    s += "**对创业者**:\n\n"
    s += "1. **选择方向**: 问自己两个问题——这件事够不够大？巨头愿不愿意进来？\n"
    s += "2. **执行速度**: 要比大公司内部团队决策更快\n"
    s += "3. **产品设计**: 追求开放和想象力，让用户能创造新范式\n"
    s += "4. **团队搭建**: 考虑'一个人 +AI 能不能干掉'，做超级个体\n"
    s += "5. **2026 机会**: AI 硬件、多模态可交互内容、ACGN 重做\n\n"
    
    s += "**对投资者**:\n\n"
    s += "1. **评估项目**: 方向够不够大？竞争壁垒是什么？\n"
    s += "2. **心态**: 对足够新足够大的方向保持乐观\n"
    s += "3. **警惕**: 线性外推会踩坑，AI 是指数增长\n"
    s += "4. **关注**: 团队进步速度比当前产品更重要\n"
    s += "5. **泡沫观**: 讨论是不是泡沫没有意义，找 winner\n\n"
    
    s += "**对职场人/个人发展**:\n\n"
    s += "1. **能力培养**: 找到 AI 无法替代的能力\n"
    s += "2. **学习方向**: 关注 AI 硬件、多模态、可交互内容\n"
    s += "3. **思维方式**: 警惕线性外推，接受指数增长\n"
    s += "4. **机会识别**: 数据产生场景、AI 语音分析、ACGN 重做\n"
    s += "5. **行动**: 用 AI 工具记录和转化数据\n\n"
    
    s += "**对产品经理**:\n\n"
    s += "1. **产品设计**: 足够开放有想象力，让人想象不到会被干掉\n"
    s += "2. **用户观**: 不要低估普通用户的创造力\n"
    s += "3. **AI 集成**: 关注微信 AI 的多模态功能\n"
    s += "4. **验证方式**: 快速验证，快速迭代\n"
    s += "5. **新范式**: 探索实时可交互内容产品\n\n"
    ...
    s += "### 🎯 本集一句话总结\n\n"
    s += "**如果你只记住一件事**:\n\n"
    s += "> 选择足够新足够大的方向（AI 硬件、多模态可交互、ACGN 重做），对早期团队保持乐观，执行速度要比大公司内部团队快，警惕线性外推思维，AI 发展是指数级的。\n\n"
    s += "---\n\n"
